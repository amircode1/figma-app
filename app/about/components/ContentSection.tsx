"use client";

import Image from 'next/image';
import { FC, useState, useEffect } from 'react';
import { getAboutPageData, type AboutPageData } from '../../lib/api';

const ContentSection: FC = () => {
  const [aboutData, setAboutData] = useState<AboutPageData | null>(null);

  useEffect(() => {
    const loadAboutData = async () => {
      const data = await getAboutPageData();
      setAboutData(data);
    };
    loadAboutData();
  }, []);

  if (!aboutData) {
    return (
      <section className="py-16 bg-[#151515]">
        <div className="container mx-auto px-6">
          <div className="text-center text-white">Loading...</div>
        </div>
      </section>
    );
  }

  return (
    <section className="py-16 bg-[#151515]">
      <div className="container mx-auto px-6">
        {/* Story Section */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-center mb-28">
          <div className="text-right order-2">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4 font-morabba">
              {aboutData.story.title}
            </h2>
            <div className="flex flex-col gap-4 text-gray-300 leading-7">
              <p className="text-sm text-gray-400 mb-2">{aboutData.story.subtitle}</p>
              {aboutData.story.content.map((paragraph, index) => (
                <p key={index} className="leading-relaxed">{paragraph}</p>
              ))}
            </div>
            <div className="mt-6 flex items-start justify-start gap-4 text-right">
                <Image
                    src={aboutData.story.quote.icon}
                    alt="quote icon"
                    width={40}
                    height={40}
                    className="text-yellow-300"
                />
                <div className="border-r-2 border-yellow-300 pr-4">
                    <p className="text-yellow-300 font-semibold text-sm">{aboutData.story.quote.text}</p>
                    <p className="text-gray-400 text-xs mt-1">{aboutData.story.quote.author}</p>
                </div>
            </div>
          </div>
          <div className="relative">
            <Image
              src={aboutData.story.image}
              alt="قهوه گارنیک"
              width={500}
              height={400}
              className="rounded-lg w-full h-auto"
              priority
            />
          </div>
        </div>
          {/* Specialty Coffee Section */}
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center mb-20">
          <div className="text-right">
            <h2 className="text-3xl md:text-4xl font-bold text-white mb-4 font-morabba">
              {aboutData.vision.title}
            </h2>
            <div className="flex flex-col gap-4 text-gray-300">
              {aboutData.vision.content.map((paragraph, index) => (
                <p key={index} className="leading-relaxed">{paragraph}</p>
              ))}
            </div>
          </div>
          <div className="flex flex-wrap gap-4">
            {aboutData.vision.images.map((image, index) => (
              <Image
                key={index}
                src={image}
                alt={`تصویر ${index + 1}`}
                width={240}
                height={200}
                className={`rounded-3xl ${index === 0 ? 'pt-8' : 'pb-8'}`}
              />
            ))}
          </div>
        </div>
      </div>
    </section>
  );
};

export default ContentSection;